...
function Index() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section className="border-y border-border bg-secondary/40"><div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 sm:grid-cols-3">{[[ShieldCheck,"Dados protegidos","Boas práticas de segurança em todas as etapas."],[Clock3,"Processo rápido","Simulação objetiva, sem papelada nesta etapa."],[BadgeCheck,"Decisão transparente","Você confere as condições antes de contratar."]].map(([Icon,title,text]) => <div key={String(title)} className="flex gap-4"><Icon className="mt-0.5 size-5 shrink-0 text-primary" /><div><h2 className="font-semibold">{title}</h2><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{text}</p></div></div>)}</div></section>
